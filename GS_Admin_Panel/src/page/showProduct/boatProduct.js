...
export default function BoatProduct() {
  ...
  useEffect(() => {
    ...
    const q = query(collection(db, "ByShip"))
  ...
  const handleDelete = async (row) => {
    ...
    setArray(newData)
    const q = query(collection(db, "ByShip"), where('name', '==', row.name))
    const docs = await getDocs(q)
    docs.forEach((querySnapshot) => {
      // console.log(querySnapshot.ref._key.path.segments[6])
      deleteDoc(doc(db, "ByShip", querySnapshot.ref._key.path.segments[6]))
    })
  }
  ...
  const SaveCangeFun = async (id) => {
    ...
    let a = null
    const q = query(collection(db, "ByShip"), where('name', '==', id))
    const docs = await getDocs(q)
    ...
    const washingtonRef = doc(db, "ByShip", a)
