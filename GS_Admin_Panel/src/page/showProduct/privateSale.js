...
export default function PrivateSale() {
  ...
  useEffect(() => {
    ...
    const q = query(collection(db, "Private"))
  ...
  const handleDelete = async (row) => {
    ...
    setArray(newData)
    const q = query(collection(db, "Private"), where('name', '==', row.name))
    const docs = await getDocs(q)
    docs.forEach((querySnapshot) => {
      // console.log(querySnapshot.ref._key.path.segments[6])
      deleteDoc(doc(db, "Private", querySnapshot.ref._key.path.segments[6]))
    })
  }
  ...
  const SaveCangeFun = async (id) => {
    ...
    let a = null
    const q = query(collection(db, "Private"), where('name', '==', id))
    const docs = await getDocs(q)
    ...
    const washingtonRef = doc(db, "Private", a)
